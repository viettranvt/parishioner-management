...
func (biz *createBusiness) Create(ctx context.Context, model *parishioner_database.Model) (*primitive.ObjectID, error) {
	return biz.store.InsertParishioner(ctx, model)
}

// this func will convert input data to parishioner model
func (biz *createBusiness) ToModel(ctx context.Context, data *parishioner_model.Parishioner) *parishioner_database.Model {
	model := &parishioner_database.Model{
		ParishName:    string_util.NormalizedData(*data.ParishName, false),
		ChristianName: string_util.NormalizedData(*data.ChristianName, false),
		Avatar:        data.Avatar,
